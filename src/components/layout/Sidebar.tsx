...
export const Sidebar = ({ currentView, setCurrentView }: SidebarProps) => {
  ...
  return (
    <aside
      className={`absolute top-0 left-0 h-screen z-50 bg-gray-900 border-r border-gray-800 shadow-2xl transition-all duration-300 ${
        isExpanded ? 'w-64' : 'w-20'
      }`}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      <div className="flex items-center h-16 px-4 overflow-hidden shrink-0">
        <svg
          width="800px"
          height="800px"
          ...
          className="w-12 h-auto text-white flex-shrink-0"
        >
          <g>
            <path
        ...
        <div
          className={`w-px h-5 bg-gray-700 mx-3 transition-all duration-500 ease-out ${
            isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
          }`}
        />

        <span
          className={`bg-gradient-to-r from-blue-400 to-cyan-300 bg-clip-text text-transparent font-bold text-xl whitespace-nowrap transition-all duration-500 ease-out ${
            isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
          }`}
        >
          DevPulse
      ...
      <div className="p-4 border-t border-gray-800">
        <div
          className={`text-xs text-gray-400 transition-all duration-500 ease-out ${
            isExpanded ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4'
          }`}
        >
          Sprint 14 • Week 2
